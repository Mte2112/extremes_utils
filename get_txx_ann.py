...
# Calculate TXx
ds_txx = ds.resample(time='1Y').max()

# Change date format to just year
ds_txx['time'] = ds_txx.time.dt.strftime("%Y")

# Rename variable to TXx
ds_txx = ds_txx.rename({'tasmax':'txxETCCDI'})

# Convert to degrees C
ds_txx['txxETCCDI'] = ds_txx['txxETCCDI'] - 273.15
...
# Area dataArray
array_area = area_grid(ds_txx['lat'], ds_txx['lon'])
# Total area
total_area = array_area.sum(['lat','lon'])
# Area-weighted temp
temp_weighted = (ds_txx['txxETCCDI']*array_area)/total_area
# weighting
...
ds_out.txxETCCDI.encoding = {'unlimited_dims': {'time'},
 'dtype': 'float32'}
...
ds_out.to_netcdf(options.outdir + '/txxETCCDI_yr_' + model + '_' + scenario + '_' + variant + '_' + years + '.nc')
