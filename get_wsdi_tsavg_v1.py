...
tsavg_thr = xr.open_dataset('tsavg_thresh.nc')

# Gettsavg_thresh diags
tsavg_thr = tsavg_thr.assign({'tsavg':tsavg_thr.tx90thresh}).drop('tx10thresh')

# Convert time coordinate from cftime to datetime (datetime is easier to apply functions to)
datetimeindex = tsavg.indexes['time'].to_datetimeindex()
...
###############################################################################################################################################
# Create master and save as netcdf
master = xr.Dataset()
master.to_netcdf('tsavg_master_hw.nc')

latlb = -79
for n in np.arange(-81, 89, 2):
    latlb = latlb + 2 
    latub = latlb + 2
    lonlb = (-178.75 - 2.5)
    for p in tsavg.lon:
        ...
        num_coord_pairs = len(tsavg_onept.lat)*len(tsavg_onept.lon)
        ...
        dummy_ds3_vs_thresh = dummy_ds3.groupby('dayofyear') - tsavg_thresh
        try:
            ...
            stacked = dummy_ds3_vs_thresh.tsavg.stack(allpoints=['lat', 'lon'])
            ...
            # Calculate thresholds by stack the coordinates and applying the get_thresh() function
            # Note- as mentioned above, the function only returns a value when the day is in the centered of the window (thus returning only one percentile value per day of year)`
            stacked2 = dummy_ds3_vs_thresh.tsavg.stack(allpoints=['lat', 'lon'])
            # apply the function over allpoints to calculate the trend at each point
            ...
            # Calculate thresholds by stack the coordinates and applying the get_thresh() function
            # Note- as mentioned above, the function only returns a value when the day is in the centered of the window (thus returning only one percentile value per day of year)`
            stacked3 = dummy_ds3_vs_thresh.tsavg.stack(allpoints=['lat', 'lon'])
            # apply the function over allpoints to calculate the trend at each point
            ...
            # Concat to ds
            #xr.concat(objs = [xr.open_dataset('tsavg_master_hw.nc'), heatwave_diagnostics], dim='concat_dim', data_vars='all', coords='minimal' ).to_netcdf('tsavg_master_hw.nc')

            # Convert to netcdf
            name = str('tsavg_' +  str(lonlb) + str(lonub) + str(latlb) + str(latub))
            heatwave_diagnostics.to_netcdf('files/' + name)
            ...
            # Compress file
           # os.system('ncks -4 -L 9 -O -h tsavg_master_hw.nc tsavg_master_hw.nc')
...
master.to_netcdf('tsavg_wsdi_master.nc')
